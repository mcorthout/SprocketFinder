...
import { ISprocket, IFrontSprocketInfo, ISprocketBike } from "../../models";
import { FrontInfoModal } from ".";
import { ReverseModal } from "../../common";

interface IFrontPartsTableProps {
    poly: Polyglot;
    language: string;
    drawing: string;
}

interface IFrontPartsTableState {
    parts: ISprocket[];
    infoSprocket: ISprocket | undefined;
    infoSprocketInfo: IFrontSprocketInfo | undefined;
    infoShowing: boolean;
...
export class FrontPartsTable extends React.Component<IFrontPartsTableProps, IFrontPartsTableState> {

    private imageBase: string = "https://service.afam.com/webshop/images/sprockets/";

    constructor(props: IFrontPartsTableProps) {
        super(props);
    ...
    public componentWillReceiveProps(nextProps: IFrontPartsTableProps) {
    ...
    public sprocketInfo = (p: ISprocket) => {
        ...
        SprocketService.GetFrontInfo(p.Item, this.props.language, this.infoLoaded);
    }

    public infoLoaded = (infos: IFrontSprocketInfo[]) => {
        const info = infos[0];
